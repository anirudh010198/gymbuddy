...
test('warm-up, rest timer (with resume), transition card, finish-confirm and cool-down all appear in order', async ({ page }) => {
  ...
  await expect(page.getByText('kg lifted')).toBeVisible()
  await expect(page.getByText('your bodyweight moved')).toBeVisible()
  await expect(page.getByText('estimated queue time avoided')).toBeVisible()
  await expect(page.getByRole('heading', { name: 'Effort mix' })).toBeVisible()
